...
func TestManagerAddWorkerWithSuccess(t *testing.T) {
	...
	expectedCommandSuffix := fmt.Sprint(`worker.test -worker -config {"name":"test-worker-1","log_level":null}`)
...
func TestManagerAddWorkersNewWorkerListenerError(t *testing.T) {
	...
	expectedCommandPrefix := "/tmp/go-build"
	expectedCommandSuffix := fmt.Sprint(`worker.test -worker -config {"name":"test-worker-3","log_level":null}`)
